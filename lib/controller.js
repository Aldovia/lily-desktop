...
const omegle = function(myTimeout, myInterests, myListener) {
      ...
      setTimeout(() => {
        ...
        request.post(
          ...
          function(err, res, data) {
            ...
            console.log('Bot: ' + msg);
            process.send({ msg: 'event', data: `Bot: ${msg}` });
...
function readEvents(data, client) {
  ...
  for (i = 0; i < data.length; i++)
    if (data[i].length == 1) client.listener.emit(data[i][0]);
    else client.listener.emit(data[i][0], data[i][1]);
}
